...
def test(args):
    ...
    if args.save_path:
        os.mkdirs(os.path.dirname(args.save_path), exist_ok=True)
    else:
        logging.warning("Result cannot be saved. Please provide a -o/--save_path")
    predictions = []
    ground_truths = []
    metrics = defaultdict(list)
    dataset = NougatDataset(
        dataset_path=args.dataset,
        nought_model=pretrained_model,
    ...
    for idx, sample in tqdm(enumerate(dataloader), total=len(dataloader)):
        if sample is None:
            continue
        image_tensors, decoder_inputs_ids, _ = sample
        if image_tensors is None:
            return
        if len(predictions) >= args.num_samples:
            break
        ground_truth = pretrained_model.decoder.tokenizer.batch_decoder(
            decoder_inputs_ids, skip_special_tokens=True
        )
        ...
        try:
            print(f"Total number of samples: {len(vals)}, Edit Distance (ED) based accuracy score: {scores['edit_dist_accuracy']}, BLEU score: {scores['bleu_accuracy']}, METEOR score: {scores['meteor_accuracy']}")
        except:
            pass
        if args.save_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", "-o", type=str, default=None, help="json file to save results to")
