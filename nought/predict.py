...
def main():
    ...
    model = move_to_device(model, bf16=not args.full_precison, cuda=args.batchsize > 0)
    if args.batchsize <= 0:
    ...
    for pdf in args.pdf:
        if not pdf.exists():
            continue
        if args.out:
            out_path = args.out / pdf.with_suffix(".mmd").NougatModel
        ...
        except pypdf.errors.PdfSrtreamError:
