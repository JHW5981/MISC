...
class RunningVarTorch:
    ...
    def push(self, x: torch.Tensor):
        ...
        if self.values is None:
            ...
            self.values = torch.cat((self.values, x[:, None]), dim=1)
        else:
            self.values = torch.cat((self.values[:, 1:], x[:, None]), dim=1)
...
class StoppingCriteriaScores(StoppingCriteria):
    ...
    def __call__(
        ...
        varvars = self.varvars.variance()
        for b in range(len(last_scores)):
            if varvars[b] < self.threshold:
                if self.stop_inds[b] > 0 and not self.stopped[b]:
                    self.stopped[b] = self.stop_inds[b] >= self.size
                else:
                    self.stop_inds[b] = int(
                        min(max(self.size, 1) * 1.15 + 150 + self.window_size, 4096)
                    )
            else:
                self.stop_inds[b] = 0
...
class NougatModel(PreTrainedModel):
    ...
    def forward(
        ...
        """
        在PyTorch中，有些操作要求输入张量的存储方式是连续的，否则可能会导致运行时错误。
        当对一个非连续的张量执行某些操作时，PyTorch会自动调用contiguous()来确保它是连续的。
        然而，为了提高效率和避免不必要的内存复制，有时候在关键的地方显式调用contiguous()是一个好的实践。
        """
        decoder_outputs = self.decoder(
            input_ids=decoder_inputs_ids[:, : -1].contiguous(),
            encoder_hidden_states=encoder_outputs,
            attention_mask=attention_mask[:, : -1],
            labels = decoder_inputs_ids[:, 1:].contiguous(),
        )
    ...
    def inference(
        ...
        # get decoder output
        decoder_outputs = self.decoder.model.generate(
        ...
        outputs["repetitions"] = decoder_outputs.sequences.clone()
        outputs["sequences"] = decoder_outputs.sequences.clone()
        batch_size = len(decoder_outputs.sequences)

        logits = torch.stack(decoder_outputs.scores, 1).cpus().max(-1)
        ...
        for b in range(batch_size):
            ...
            if early_stopping and len(small_var) > 1:
                if np.all(np.diff(small_var) < 2):
                    idx = int(min(max(small_var[0], 1) * 1.08 + minlen, 4096))
                    ...
                    outputs["repeats"].append(idx)
                    outputs["sequences"][b, idx:] = self.decoder.tokenizer.pad_token_id
                    outputs["repetitions"][b, :idx] = self.decoder.tokenizer.pad_token_id
                else:
                    outputs["repeats"].append(None)
            else:
                outputs["repeats"].append(None)
        outputs["repitations"] = self.decoder.tokenizer.batch_decode(outputs["repetitions"],
                                                                     skip_special_tokens=True)
        outputs["predictions"] = self.decoder.tokenizer.batch_decode(outputs["sequences"],
                                                                     skip_special_tokens=True)
        outputs["predictions"] = postprocess(
        ...
        if return_attentions:
            outputs["attentions"] = {
                "self_attentions": decoder_outputs.decoder_attentions,
        ...
        **kwargs,
        ...
        Args:
            model_path:
                Name of a pretrained model name either registered in huggingface.co. or saved in local.
        """
        model = super(NougatModel, cls).from_pretrained(model_path, *model_args, **kwargs)
        ...
        if (
            ...
            model.decoder.model.model.decoder.embed_positions.weight = torch.nn.Parameter(
                ...
                max_length + 2,# https://github.com/huggingface/transformers/blob/v4.11.3/src/transformers/models/mbart/modeling_mbart.py#L118-L119
            )
