...
def download_as_bytes_with_progress(url: str, name: str = None) -> bytes:
    ...
    with tqdm.tqdm(
        ...
        for chunck in resp.iter_content(chunk_size=65536):
...
def download_checkpoint(checkpoint: Path, model_tag: str = MODEL_TAG):
    ...
    """
    print("downloading nougat checkpoint version", model_tag, "to_path", checkpoint)
    files = [
        "config.json",
        "pytorch_model.bin",
...
def get_checkpoint(
    ...
    """
    checkpoint = Path(
        checkpoint_path or os.environ.get("NOUAGT_CHECKPOINT", torch_hub(model_tag))
    )
